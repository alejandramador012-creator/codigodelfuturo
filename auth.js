...
/* ════════════════════════════════════════
   AUTO-INJECT NAV SI YA HAY SESIÓN (landing)
════════════════════════════════════════ */
(function () {
  const onLanding = ['', '/', 'index.html'].some(s => window.location.pathname.endsWith(s));
  if (!onLanding) return;

  // Usar localStorage directamente — sin depender de Firebase onAuthStateChanged
  var u = null;
  try { u = JSON.parse(localStorage.getItem('cdf_user')); } catch(e) {}
  if (!u || !u.uid) return;

  function injectDashNav() {
    const navActions = document.querySelector('.nav-actions');
    if (!navActions) return;
    const firstName = (u.name || u.email || 'Miembro').split(' ')[0];
